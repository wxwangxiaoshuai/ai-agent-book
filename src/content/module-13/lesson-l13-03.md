...
from openai import OpenAI
client = OpenAI()
# TOOL_MAP / TOOLS / system / user() / tool_msg() 为既有封装
...
def run_agent(question: str):
    ...
    with tracer.start_as_current_span("agent_run") as trace_span:
        ...
        for step in range(MAX_STEPS):
            ...
            # 子 span 可记：llm.latency_ms = latency_ms
            ...
            for tc in msg.tool_calls:
                ...
                result = TOOL_MAP[tc.function.name](**args)  # 子 span
...
**告警**：基于这些指标设告警——p99 延迟突涨、平均步数飙升、护栏触发激增——都该触发告警人工介入。
...
- 与评测(L13-02)/护栏(L13-04)构成质量保障铁三角：上线前评测+运行时trace+运行时护栏
