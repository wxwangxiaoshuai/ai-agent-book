...
import tiktoken, chromadb, json, numpy as np
...
对话：""" + dialog
    ...
    def _upsert(self, user_id, fact_obj):
        ...
        old = self.col.query(query_texts=[new_fact], n_results=1,
        ...
        if relation == "duplicate": return "skipped"
        if relation == "complementary":
            merged = self._merge(old_doc, new_fact)
            self.col.update(ids=old_ids, documents=[merged])
            return "merged"
        # contradictory：新覆盖旧，旧归档
        self.col.update(ids=old_ids, metadatas=[{"status":"archived"}])
...
class PrivateRAG:
    """私有文档检索：BM25 + 向量混合 + Cross-encoder 重排"""
    ...
    def index(self, doc_id, text, chunk_size=400):
        """索引：分块 + embedding + BM25 语料"""
        ...
        for i, c in enumerate(chunks):
            ...
            self.bm25_corpus.append((cid, c.split()))
    ...
    def retrieve(self, query, top_k=5):
        ...
        # 合并去重
        cand_ids = list(dict.fromkeys(vec_ids + bm_top))
        docs = []
        for cid in cand_ids:
            r = self.col.get(ids=[cid])
            if r["documents"]:
                docs.append((cid, r["metadatas"][0]["text"], r["metadatas"][0]["doc_id"]))
        # Cross-encoder 重排（M4）
        try:
            ...
            docs = docs[:top_k]
        return docs  # [(chunk_id, text, doc_id), ...]
```
...
class KnowledgeButler:
    ...
    def __init__(self):
        ...
        self.window.set_system(
            ...
            "每个关键结论用 [^doc_id] 标注来源。文档中没有的说明'资料中未提及'。")
...
import pytest
from src.knowledge_butler import LongTermMemory
...
- 私有 RAG 复用 M4：BM25+向量混合召回 → Cross-encoder 重排 → 带引用回答
