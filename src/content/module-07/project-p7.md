            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        ...
                        logger.info(f"重试 {attempt+1}/{max_retries} ({delay:.1f}s): {fn.__name__} - {e}")
                        time.sleep(delay)
                    else:
                        logger.error(f"重试耗尽: {fn.__name__} - {e}")
...
model_chain.add(lambda **kw: "AI 服务暂时不可用，请稍后重试。", "静态兜底")
...
class ResilientAgent:
    ...
    def run(self, question: str, agent_id: str = None) -> str:
        ...
        for step in range(start_step, self.max_steps):
            try:
                # 用降级链调 LLM
                response = self.model_chain.execute(messages=messages)
                ...
                return f"Agent 在 step {step+1} 失败: {e}。恢复 ID: {agent_id}"
...
from src.resilient import resilient, CircuitBreaker, FallbackChain
...
class TestRetry:
    ...
    def test_fallback_on_exhaustion(self):
        def static_fallback():
            return "降级回复"
        @resilient(max_retries=2, base_delay=0.01, fallback=static_fallback)
        def always_fail():
            raise ConnectionError("持续故障")
...
class TestCircuitBreaker:
    ...
    def test_half_open_after_recovery(self):
        ...
        assert breaker.state.value == "open"
        time.sleep(0.02)
        assert breaker.can_execute()  # 进入 half_open

class TestFallbackChain:
    def test_falls_through_to_second(self):
        chain = FallbackChain("test")
        chain.add(lambda: (_ for _ in ()).throw(Exception("失败")), "primary")
        chain.add(lambda: "备用成功", "backup")
        assert chain.execute() == "备用成功"
...
- 弹性框架 = 重试 + 熔断 + Checkpoint + 降级，一键装饰器即插即用
- `@resilient` 装饰器：指数退避重试 → 熔断保护 → 降级到 fallback
- Checkpoint：每步保存状态，失败后从断点恢复
- 降级链：主模型 → 备用模型 → 静态兜底，用户无感切换
- 压力测试验证：模拟 429/503/超时，确认重试和降级按预期触发
