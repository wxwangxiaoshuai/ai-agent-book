...
构建多模态内容分析 Agent：
- 多模态输入路由：自动识别图片/语音/视频（及可选 PDF）并分流
- 图片分析：内容描述 + OCR + 图表数据提取
- 语音分析：转写 + 摘要 + 情感分析
- 视频分析：关键帧提取 + 时序事件标注 + 视频摘要
- （进阶）PDF：多页文档结构化提取（L12-05）
    ...
    resp = client.chat.completions.create(
        ...
        messages=[{"role": "user", "content": [
            {"type": "text", "text":
                ...
                '{"description":"内容描述","has_text":true,"ocr_text":"...",'
                '"has_chart":false,"chart_data":null,"confidence":"high|medium|low"}。'
                "读不准的标 [?] 或 null，不要猜。"},
...
def process_video(video_path: str, max_frames: int = 20) -> dict:
    ...
    content = [{"type": "text", "text":
        "这些是视频按时间顺序的关键帧。只输出 JSON 对象，字段示例："
        '{"events":[{"time_sec":0,"description":"..."}],"summary":"整体摘要(2句)"}。'
        "标注每个事件的大致时间。"}]
...
def sample_keyframes(video_path: str, max_n: int) -> list:
    ...
    frames, prev_gray, last_ts = [], None, -1.0
    while True:
        ret, frame = cap.read()
        if not ret: break
...
def process_pdf(pdf_path: str, max_pages: int = 10) -> dict:
    ...
    content = [{"type": "text", "text":
        "理解这份多页文档。只输出 JSON 对象，字段示例："
        '{"doc_type":"发票|合同|报告|...","summary":"内容摘要",'
        '"key_fields":{"字段":"值"},"source_pages":{"字段":1}}。认不准标 [?]。'}]
    for img in pages:
...
def assemble_report(modality: str, content_result: dict) -> dict:
    ...
    report = {
        "modality": modality,
        # 图片无 summary 字段时回退 description
        "summary": content_result.get("summary") or content_result.get("description") or "",
        "data": content_result,
    ...
    if modality == "image":
        ...
        report["content"] = {
            ...
            "frame_count": content_result.get("frame_count", 0),
            "event_count": len(content_result.get("events", [])),
        }
    elif modality == "pdf":
        report["content"] = {
...
- 图片处理：一次多任务调用（描述+OCR+图表），结构化输出 + 校验
