...
先用交互面板对比「纯文本转述 vs 多模态直读」的信息损失，再进入选型与 API：
...
def vision_base64(image_path: str, question: str) -> str:
    ...
    mime = {".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg",
            ".webp": "image/webp"}.get(Path(image_path).suffix.lower(), "image/png")
...
# 文件上传方式（大图/私有）：先 Files API 上传，再按当前文档的 file 引用格式入消息
# uploaded = client.files.create(file=open(image_path, "rb"), purpose="vision")
# 具体 content part 字段名以 OpenAI 视觉文档为准（版本迭代较快）
